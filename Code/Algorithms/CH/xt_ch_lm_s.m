function [e, iter, Ns] = xt_ch_lm_s(M, mask, r, N, max_iter, tol, disp_level)

% LM_S in the paper (Damped RW0)
...
warning('off', 'MATLAB:nearlySingularMatrix'); % No ambiguity constraint present
...
for iter = 1 : max_iter
  [b, H] = get_b_H0(M, NiQ, NiR, r, store);
  
  for trial = 1 : max_trials
    delta = (H + lambda * speye(nr)) \ b;
    %delta=inv(H+lambda*diag(diag(H)))*b;
    
    % variation=b'*delta;
    % K=mtr(delta,n-r,r);
    [N_new, ~] = qr(N + reshape(delta, n, r), 0);
...
function [b, H] = get_b_H0(M, NiQ, NiR, r, store)
