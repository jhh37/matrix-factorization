function [U, W, stats] = xt_nb_rtrmc(problem, opts, U0)
...
if exist('manopt_version', 'file') ~= 2
  error(['Please add the Manopt toolbox to your path first. ' ...
    ...
    'the Manopt_1.0.6 folder packaged with this release, ' ...
...
% checkgradient(optproblem);
% checkhessian(optproblem);
...
%% Define the cost function
  function [f, store] = cost(U, store)
    ...
    if ~isfield(store, 'val')
      ...
      [W, store] = xt_nb_lsqfit(problem, U, store);
      store.W = W;
      
      UW = xt_nb_spmaskmult(U, W, I, J);
      store.UW = UW;
      
      store.val = ( sum((C.*(UW-X)).^2)    ...
        + lambda.^2*sum(W(:).^2) ...
        - lambda.^2*sum(UW.^2)   ) / k;
    end
    f = store.val;
...
%% Define the gradient function
  function [grad, store] = gradient(U, store)
    ...
    if ~isfield(store, 'grad')
      ...
      store.grad = 2 * (xt_nb_multsparsefull(RU, W.', mask) + U*sqlaWWt)/k;
...
%% Define the Hessian function
  function [hess, store] = hessian(U, H, store)
    ...
    [W, dW, store] = xt_nb_lsqfit(problem, U, H, store);
    UdW = xt_nb_spmaskmult(U, dW, I, J);
    ...
    hess = xt_nb_multsparsefull(Chat.*(HW + UdW), W.', mask);
    hess = hess - U*(U.'*hess);
    hess = hess + xt_nb_multsparsefull(RU, dW.', mask);
    hess = hess + H*sqlaWWt;
