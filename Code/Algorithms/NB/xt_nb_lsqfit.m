function [W, out2, out3] = xt_nb_lsqfit(problem, U, in3, in4)
...
% SEE ALSO: buildproblem rtrmcobjective
    ...
    if ~isfield(compumem, 'Achol')
        compumem.Achol = xt_nb_buildmatrix(problem, U);
    end
    Achol = compumem.Achol;

    % Solve LSQ system block by block.
    if ~isfield(compumem, 'W')
        rhs = xt_nb_multfullsparse(U.', (C.^2.*X), mask);
        compumem.W = xt_nb_blocksolve(Achol, rhs);
    end
    W = compumem.W;
...
function Achol = xt_nb_buildmatrix(problem, U)
...
% A is defined in the accompanying paper.
    ...
    mask = problem.mask;
    xt_nb_setsparseentries(mask, sqrt(Chat));
    
    Achol = xt_nb_spbuildmatrix(mask, U, lambda^2);
    
end

function X = xt_nb_blocksolve(R, B)
